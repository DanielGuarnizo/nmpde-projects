...
# # This script runs the simulation with multiple different preconditioners and
# # analyzes the output to provide a concise summary.
# # It now uses the same robust execution pattern as the successful figure_8 script.
...
# # --- Step 4: Create the run_simulation function (pattern from the working script) ---
# run_simulation() {
#     local precon_name=$1
#     local output_dir=$2
#     local log_file=$3
...
#     # DIRECT EXECUTION: This is the robust method from the successful script.
...
#     end_time=$(date +%s.%N)
#     wall_time=$(echo "$end_time - $start_time" | bc -l) # Added -l for floating point

#     # --- Step 7: Analyze the log file ---
#     if grep -q "Simulation finished." "$LOG_FILE"; then
#         TOTAL_IT=$(grep "Total linear iterations:" "$LOG_FILE" | awk '{print $4}')
#         AVG_IT=$(grep "Average iterations per solve:" "$LOG_FILE" | awk '{print $5}')
#         status_messages[$PRECON]=$(printf "SUCCESS | Time: %6.2f s | Total It: %-7s | Avg It: %s" "$wall_time" "$TOTAL_IT" "$AVG_IT")
#         echo "RESULT: ${status_messages[$PRECON]}"
#     else
#         echo "!!! SIMULATION FAILED. See last 10 lines of log:"
#         tail -n 10 "$LOG_FILE"
#         status_messages[$PRECON]=$(printf "FAILED  | Time: %6.2f s | Check log for errors." "$wall_time")
#         echo "RESULT: ${status_messages[$PRECON]}"
#     fi
# done

# # --- Step 8: Print the final summary table ---
# echo ""
# echo "================================================================================"
...
# done
# echo "================================================================================"
# echo "Benchmark finished. Detailed logs are in: $RESULTS_BASE_DIR"
